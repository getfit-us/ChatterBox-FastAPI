...
def main():
    ...
    parser.add_argument(
        "input", type=str, help="Path to input (a sample or folder of samples)."
    )
    parser.add_argument(
        "target_speaker", type=str, help="Path to the sample for the target speaker."
    )
    parser.add_argument("-o", "--output_folder", type=str, default="vc_outputs")
    parser.add_argument("-g", "--gpu_id", type=int, default=None)
    parser.add_argument(
        "-m", "--mps", action="store_true", help="Use MPS (Metal) on macOS"
    )
...
def convert_to_wav(audio_file_path, new_audio_file_path):
    audio, sr = librosa.load(audio_file_path, sr=S3_SR)
    sf.write(new_audio_file_path, audio, sr)
